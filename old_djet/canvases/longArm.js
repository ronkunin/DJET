...
function longArm_preload() {
  longArm_start_img = loadImage('../old_djet/images/longArm_startimg.png');
    longArm_background = loadImage('../old_djet/images/longArm_background.png');
    longArm_modes = [
        {name: "arm", headImg: loadImage('../old_djet/images/longArm_arm.png'), headR: PI*0.75, bodyImg: loadImage('../old_djet/images/longArm_body.png'), sizeH: 1.2, sizeW: 1},
        {name: "plane", headImg: loadImage('../old_djet/images/longArm_b707.png'), headR: PI, bodyImg: loadImage('../old_djet/images/longArm_f35.png'), sizeH: 1, sizeW: 0.8},
    ];
    longArm_food_types_imgs = [
        loadImage('../old_djet/images/longArm_food_m16.png'),
        loadImage('../old_djet/images/longArm_food_missle.png'),
        loadImage('../old_djet/images/longArm_food_uav.png'),
    ];
    longArm_end_imgs = [loadImage('../old_djet/images/longArm_endimg1.png'),loadImage('../old_djet/images/longArm_endimg2.png'),loadImage('../old_djet/images/longArm_endimg3.png')];
...
function longArm_keyPressed(keyCode) {
  if(game_status == 32 || game_status == 30){
    if(keyCode==97)
      longArm_modifier= "NORMAL";
    else if(keyCode==98)
      longArm_modifier= "GOD";
    else if(keyCode==99)
      longArm_modifier= "DiChange";
  }
      ...
      snake.setDir(0, -1);
        ...
        if (toggle && longArm_modifier == "GOD") {
          frameRate(0);
        } else {
          frameRate(10);
        }
...
function longArm_draw() {
    ...
    if(game_status == 30) {
        ...
        textAlign(CENTER);
        textSize(20);
        //text("ניתן להחליף מצבי משחק באמצעות המספרים בפאד", 250, 150);
       // text(longArm_modifier, 250, 170);
  ...
  class Snake {
    ...
    setDir(x, y) {
      if((x * this.xdir == -1 || y * this.ydir == -1) && this.len > 1 && longArm_modifier == "NORMAL")
        return;
    ...
    grow() {
      if (longArm_modifier == "DiChange"){
        ...
        this.body[this.body.length - 1] = this.body[0].copy()
      }
      this.len++;
    ...
    endGame() {
      // Normal Modifier
      if (longArm_modifier == "NORMAL" || longArm_modifier == "DiChange") {
        let head = this.body[this.body.length - 1];
      ...
      // God Mofifier
      if (longArm_modifier == "GOD") {
        let head = this.body[this.body.length - 1] 
        if (head.x > LA_w -1) {
          head.x = 0;
          this.body[0] = createVector(-1, head.y);
        }
        else if (head.x < 0) {
          head.x = LA_w;
          this.body[0] = createVector(LA_w, head.y);
        }
        else if (head.y > LA_h - 1) {
          head.y = 0;
          this.body[0] = createVector(head.x, -1);
        }
        else if (head.y < 0) {
          head.y = LA_h;
          this.body[0] = createVector(head.x, LA_h);
        }
        return false;
    }
    }
    show() {
      for (let i = 0; i < this.body.length; i++) {
        ...
        if (i === this.body.length - 1) {
            ...
            console.log(longArm_mode.headR);
        } else {
